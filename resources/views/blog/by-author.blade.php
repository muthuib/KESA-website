@extends('layouts.app')

@section('content')
<div class="container py-5">
  <h2 class="mb-4 text-center fw-bold" style="margin-top: 70px; font-size: 18px;">
    📝 Blogs by: {{ $author }} 
    <span class="badge bg-info text-dark fs-6 ms-2">{{ $blogs->total() }}</span>
</h2>

    @if($blogs->count())
        <div class="row g-4">
            @foreach($blogs as $item)
                <div class="col-md-4 d-flex">
                <div class="card shadow-sm border-0 rounded-4 w-100 hover-shadow transition d-flex flex-column">
                    
                    @if($item->image)
                        <a href="{{ route('blog.show', $item->slug) }}">
                            <img src="{{ asset($item->image) }}" class="card-img-top blog-img bg-light">
                        </a>
                        @if(!empty($item->name))
                            <p class="small mb-1 text-secondary">{{ $item->name }}</p>
                        @endif
                    @endif

                    <div class="card-body d-flex flex-column px-4 pt-3 pb-4">
                        <h6 class="card-title fw-semibold mb-2 small">
                            <a href="{{ route('blog.show', $item->slug) }}" class="text-decoration-none text-dark">
                                {{ $item->title }}
                            </a>
                        </h6>

                        <p class="text-muted mb-3 small">
                            <i class="fas fa-calendar-alt me-1"></i>
                            {{ \Carbon\Carbon::parse($item->date)->format('M d, Y') }}
                        </p>

                        <p class="text-secondary mb-2" style="font-size: 0.9rem;">
                            <i class="bi bi-person"></i> Author: 
                              @php
                                // Split authors into an array (trim spaces as well)
                                $authors = array_map('trim', explode(',', $item->author ?? 'Admin'));
                            @endphp

                            @foreach($authors as $index => $author)
                                <a href="{{ route('blog.byAuthor', ['author' => $author]) }}" 
                                class="text-primary text-decoration-none">
                                    <strong>{{ $author }}</strong>
                                </a>
                                @if($index < count($authors) - 1)
                                    , 
                                @endif
                            @endforeach
                            
                            &nbsp; | &nbsp;
                            📂 Category: <strong>{{ $item->category ?? 'Uncategorized' }}</strong>
                        </p>

                        <div class="card-text text-muted small mb-4" style="flex: 1;">
                            {{ Str::limit(strip_tags($item->content), 130, '...') }}
                        </div>

                        <a href="{{ route('blog.show', $item->slug) }}" class="btn btn-sm btn-outline-primary w-100 mt-auto rounded-pill">
                            <i class="fas fa-eye me-1"></i> Read More
                        </a>
                    </div>
                </div>
            </div>

            @endforeach
        </div>

        <!-- Pagination -->
        <div class="mt-5">
            <nav aria-label="Page navigation" class="d-flex justify-content-center">
                {{ $blogs->links('pagination::bootstrap-5') }}
            </nav>
        </div>
    @else
        <div class="alert alert-info text-center mt-5">
            <i class="fas fa-info-circle me-2"></i> No blogs found for this author.
        </div>
    @endif
</div>
@endsection

@section('styles')
<style>
    .hover-shadow:hover {
        transform: translateY(-3px);
        box-shadow: 0 10px 20px rgba(0, 0, 0, 0.08);
        transition: all 0.3s ease-in-out;
    }

    .card-title {
        font-size: 0.95rem;
    }

    .card-text {
        font-size: 0.87rem;
    }

    .blog-img {
        height: 200px;
        object-fit: contain;
        padding: 10px;
        width: auto;
    }

    @media (max-width: 992px) {
        .blog-img {
            width: 100%;
            height: auto;
            object-fit: cover;
            padding: 0;
            border-top-left-radius: 1rem;
            border-top-right-radius: 1rem;
        }
    }

    @media (max-width: 768px) {
        .input-group .form-control {
            border-radius: 50px 0 0 50px !important;
        }
        .input-group .btn {
            border-radius: 0 50px 50px 0 !important;
        }
    }
</style>
@endsection
