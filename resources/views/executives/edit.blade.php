...
<div class="container my-5" style="margin-top: 90px;">
    <h2 class="text-center mb-4">Edit Executive Member</h2>
    ...
    <form action="{{ route('executives.update', $executive->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        
        <div class="mb-3">
            <label for="name" class="form-label">Full Name</label>
            <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $executive->name) }}" required>
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="designation" id="designation" class="form-control" value="{{ old('designation', $executive->designation) }}" required>
            @error('designation')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
<!--         
        ...
        <div class="mb-3">
            ...
            @if($executive->image)
                <div class="mb-2">
                    <img src="{{ asset($executive->image) }}" alt="{{ $executive->name }}" class="img-thumbnail" style="max-width: 200px;">
                </div>
            @endif
