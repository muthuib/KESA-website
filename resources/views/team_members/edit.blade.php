...
<div class="container my-5" style="margin-top: 90px;">
    ...
    <form action="{{ route('team-members.update', $teamMember->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="designation" id="designation" class="form-control" value="{{ old('designation', $teamMember->designation) }}" required>
            @error('designation')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="url" name="cv_link" id="cv_link" class="form-control" value="{{ old('cv_link', $teamMember->cv_link) }}">
            @error('cv_link')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div> -->
        ...
        <div class="mb-3">
            ...
            @if($teamMember->image)
                <div class="mb-2">
                    <img src="{{ asset($teamMember->image) }}" alt="{{ $teamMember->name }}" class="img-thumbnail" style="max-width: 200px;">
                </div>
            @endif
